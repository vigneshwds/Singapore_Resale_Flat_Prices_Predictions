...
def statistics(): 
    ...
    with col1:
        ...
        data={'Outliers':['Total number of outliers in Singapore DataSet', 'Percentage of Outlier'], 'Values':[8633, 3.86]}
        st.dataframe(data)

        st.subheader('Average number of Flats')
        st.markdown('**Singapore Average Number of Flats in each Street : 23**')
    ...
    with col3:
        st.subheader('Feature Sknewness and Kurtosis values')
...
img = st.image(Image.open('C:\Capstone files\Singapore\header.png'))#, width=1020)
...
st.sidebar.header('Regressor Model!')
st.sidebar.markdown('Vignesh')
